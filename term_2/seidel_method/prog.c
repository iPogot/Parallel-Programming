...
int calc(int first_row, int last_row, double **field, double **rcv_field, int step) {
    ...
    for(int i = first_row; i <= last_row; i++) {
        for(int j = 1; j < ROW_NUM - 1; j++) {
            field[i][j] = (field[i][j - 1] + rcv_field[i][j + 1] + field[i - 1][j] + rcv_field[i + 1][j]) / 4;
        }   
    }   
}
...
int jacobi_calc(int rank, int size, double** prev_field, double** new_field, int n_steps) {
    ...
    while(cur_step < n_steps) {
        ...
        //printf("RANK = %d,\nfirst_calc_row = %d,\nlast_calc_row = %d,\nCurent_step = %d\n", rank, first_calc_row, last_calc_row, cur_step);

        //printf("RANK = %d,\nfirst_calc_row = %d,\nlast_calc_row = %d,\nCurent_step = %d,\nfirst_rcv_row = %d,\nlast_rcv_row = %d\n\n",
         //                    rank, first_calc_row, last_calc_row, cur_step, first_rcv_row, last_rcv_row);
        cur_step += size;
    }
